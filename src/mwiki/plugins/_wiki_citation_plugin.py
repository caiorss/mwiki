"""Extends markdown_it_py parser to support Pandoc-like citation references.
Citation syntax.

Examples: 
+  $[ @citationKey1 ]
+  $[@citationKey1; @citationKey2]
+  $[@citationKey2, page:10-20; @citationKey2, chapter:10]
"""
...
RULE_NAME = "wiki_citation"
MAIN_DELIMITER = "double_square_brackets"

def wiki_citation_plugin(
        ...
        "inline": [
           {
                #### "name": "math_inline",
                 "name": "wiki_citation"
               , "rex": re.compile(r"\$\[(.+?)\]")
               , "tmpl": """<a href="/dummy-hyperlink">DUMMY</a>"""
               , "tag": "$["
               # , "pre": wikilink_pre
           }
