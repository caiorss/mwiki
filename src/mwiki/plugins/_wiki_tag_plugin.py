"""Implement syntax of Obsidian's pound-sign tag.

For instance the Obsidian's Markdown tag #math when 
clicked searches for all articles containing this tag
'#math'.
"""
...
RULE_NAME = "wiki_tag"
MAIN_DELIMITER = "number_sign" # '#' - number sign character 

def wiki_tag_plugin(
    md: MarkdownIt, delimiters: str = MAIN_DELIMITER, macros: Any = None
) -> None:
    """Implements Obsidian's tag syntax using '#' number sign character.

    This plugin regards the following REGEX pattern as tag.
    '#(\S+)'
    """
...
rules: dict[str, dict[str, list[RuleDictType]]] = {
    ...
    MAIN_DELIMITER: {
        "inline": [
           {
                #### "name": "math_inline",
                 "name": "wiki_tag_inline"
               , "rex": re.compile(r"(#\S+)")
               , "tmpl": """<a href="/pages?search=%23{0}" class="link-internal" >{0}</a>"""
               , "tag": "#"
               ## , "pre": wikilink_pre
           }
