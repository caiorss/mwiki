...
+ Change footnote syntax in order to avoid parsing ambiguity. The MWiki syntax for footnote was changed from `^{footnote here}` to `^{@ footnote here @}`. The Obsidian's compatibility syntax `^[footnote here]` for footnotes still remains the same.
+ Implement command $ mwiki manage --shell for flask db shell.
+ The Index.md page is automatically created if it does not exist yet.
...
+ Add flag `--clean-cache` to command `$ mwiki server` for cleaning the cache directory `$MWIKI_DIRECTORY/.data/cache` containing pages compiled to html. Cleaning the cache may be useful for changing the site favicon without manually deleting the cached directory.
+ Divide settings forms into sections for improving organization and usability using `<h2>` headings for each section. The settings were grouped as general settings, language settings, editor settings and font settings.
+ Implement default content language in language in the settings form. This language setting form allows administrators to set the default content language, which is rendered as `<html lang="$(ISO-LANGUAGE-CODE)">`. For instance, if the default content language is set to French, the html language declaration will be `<html lang="fr">`. This HTML attribute is useful for search engine and screen readers.
+ Implement frontmatter's directive 'language', for overriding the default content language of the current document. For instance, if the default content language is English and the content language of the current document (MWiki page) is in German (Deustch), it is possible to override the default content language by adding the directive `language: de` in the frontmatter. Where "de" is the ISO language code of the German language. This language declaration is rendered to `<html lang="de">`. 
+ Implement command line swtich `--content-language` for the static websiste generator command `$ mwiki export`. This switch allows declaring the ISO language code of the default content language in the lang attribute `<html lang="$LANGUAGE-CODE">`. If no content language is defined, the content language code will "en" (English). If any markdown page uses a different language, it can be declared using with the frontmatter's directive `language: $(LANGUAGE-CODE)`. For instance, `language: es` for declaring that the page content is in Spanish.
...
+ Add Flask-Migrate Python-Package dependecy.
...
+ Document preview now supports KaTeX, which provides faster math rendering and page loading.
