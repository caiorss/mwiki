...
+ Implement default content language in language in the settings form. This language setting form allows administrators to set the default content language, which is rendered as `<html lang="$(ISO-LANGUAGE-CODE)">`. For instance, if the default content language is set to French, the html language declaration will be `<html lang="fr">`. 
